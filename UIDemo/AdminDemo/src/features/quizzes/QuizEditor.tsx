...
import { ChevronLeft, Plus, Trash2, GripVertical, Save, Brain, Settings, FileText, Info, X, Activity } from 'lucide-react';
import { api } from '../../api';
...
export const QuizEditor: React.FC = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      alert('Assessment blueprint synchronized.');
    }
  });
  ...
  const addItem = (type: 'Paragraph' | 'Question') => {
    const newItem = {
      ...
      gradingData: type === 'Question' ? { choices: [], correctChoiceId: null, shuffleOptions: true } : null
    };
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-3">
           <button 
             ...
             className={clsx("btn gap-2 px-6", isPreviewMode ? "btn-primary bg-slate-900 text-white" : "btn-secondary")}
           >
             {isPreviewMode ? <Activity size={16} /> : <Settings size={16} />}
             <span>{isPreviewMode ? 'Blueprint Mode' : 'Live Preview'}</span>
           </button>
           <button 
             onClick={() => saveMutation.mutate()}
             disabled={saveMutation.isPending}
             className="btn btn-primary bg-primary-600 text-white gap-2 px-8 shadow-lg shadow-primary-500/20"
           >
      ...
      {!isPreviewMode ? (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
              ...
              {items.map((item, index) => (
                ...
                >
                  ...
                  <div className="p-8 space-y-6">
                    ...
                    {item.type === 1 && (
                      <div className="pt-6 border-t border-slate-50 dark:border-slate-900 space-y-6 text-left">
                          <div className="flex items-center gap-8">
                            ...
                            <div className="space-y-2">
                              ...
                              <select className="h-8 py-0 text-xs font-bold uppercase tracking-widest bg-transparent">
          ...
          <div className="space-y-8">
            ...
            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-4 text-left">
        ...
        <div className="max-w-3xl mx-auto space-y-12 bg-white dark:bg-slate-950 p-16 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden">
           <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-primary-500 to-indigo-500" />
           <div className="space-y-3 border-b border-slate-100 dark:border-slate-900 pb-10 text-left">
              <div className="flex items-center gap-2 text-primary-600 font-bold text-[10px] uppercase tracking-[0.2em]">
                 <Brain size={12} />
                 <span>Validated Assessment</span>
              </div>
              <h2 className="text-3xl font-black tracking-tight">{quiz?.title}</h2>
              <p className="text-slate-500 font-medium leading-relaxed">{quiz?.description || 'Follow instructions for each unit carefully.'}</p>
           </div>
           
           <div className="space-y-20">
              {items.map((item, index) => (
                <div key={item.id} className="space-y-8 text-left">
                   <div className="flex items-start gap-6">
                      <span className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center text-sm font-black shrink-0 border border-slate-200 dark:border-slate-800 shadow-sm">{index + 1}</span>
                      <div className="space-y-6 flex-1 pt-1">
                         <h3 className={clsx("font-bold leading-tight", item.type === 0 ? "text-xl text-slate-600 italic" : "text-2xl text-slate-900 dark:text-white")}>
                           {item.renderData.title}
                         </h3>
                         {item.type === 1 && (
                           <div className="grid grid-cols-1 gap-4">
                              {item.gradingData?.choices?.map((choice: any) => (
                                <div key={choice.id} className="group p-5 rounded-2xl border border-slate-100 dark:border-slate-900 hover:border-primary-500/50 hover:bg-primary-50/5 dark:hover:bg-primary-900/10 transition-all cursor-pointer flex items-center gap-4">
                                   <div className="w-5 h-5 rounded-full border-2 border-slate-200 dark:border-slate-800 group-hover:border-primary-500 transition-colors" />
                                   <span className="text-base font-semibold text-slate-700 dark:text-slate-300">{choice.renderData.label}</span>
                                </div>
                              ))}
                           </div>
                         )}
                      ...
                      {item.type === 1 && (
                        <div className="shrink-0 pt-2">
                           <div className="px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-900 text-[10px] font-black uppercase text-slate-500 border border-slate-200 dark:border-slate-800">
                             {item.points} PTS
                           </div>
                        </div>
                      )}
