...
[AlphaZero.Modules.Tenants]
  ├── Value Object: TenantBranding (PrimaryColor, SecondaryColor, LogoUrl, DarkModeLogoUrl)
...
- [ ] **Task 1.1: Create `TenantBranding` Value Object**
...
- [ ] **Task 1.2: Refactor `Tenant.cs` Aggregate Root**
...
- [ ] **Task 1.3: EF Core Entity Configuration & Migration**
  - **Path:** `src/alphazero-api/Modules/Tenants/Infrastructure/Persistence/Configurations/TenantConfiguration.cs`
  - **Details:**
    - Map `TenantBranding` as an owned entity (`builder.OwnsOne(x => x.Branding)`) or dedicated columns (`Branding_PrimaryColor`, `Branding_SecondaryColor`, `Branding_LogoUrl`, `Branding_DarkModeLogoUrl`, `Branding_FaviconUrl`).
    - Generate EF Core database migration for PostgreSQL.
...
- [ ] **Task 2.1: Update `UpdateTenantCommand` & Handler**
...
- [ ] **Task 2.2: Upgrade `LookupTenantEndpoint` & Response DTO**
...
- [ ] **Task 2.3: In-Memory Domain Event Consumer for Cache Invalidation**
  ...
  - **Details:**
    ...
    - Invalidate Redis tenant cache entry `tenant:subdomain:{subdomain}`.
...
- [ ] **Task 3.1: Create Algorithmic Color & Contrast Utility**
  ...
  - **Details:**
    ...
    - Implement `generateTenantCssVariables(primaryHex: string, secondaryHex?: string | null): string`:
      ...
      - Injects `--primary`, `--primary-foreground`, `--ring`, `--secondary`, `--secondary-foreground`, `--accent`, `--accent-foreground`.
  - **Acceptance Criteria:** Unit tests in vitest verifying contrast ratio decisions for yellow, navy, crimson, and black.

- [ ] **Task 3.2: Verify Tailwind v4 Theme Tokens**
...
- [ ] **Task 4.1: Edge Subdomain Resolution Middleware**
  - **Path:** `src/alphazero-frontend/apps/app/middleware.ts` (or `proxy.ts`)
  - **Details:**
    - Read `host` header from `NextRequest`.
...
- [ ] **Task 4.2: Dynamic Theme Injection in Root Layout**
...
- [ ] **Task 4.3: Add Revalidation API Route**
...
- [ ] **Task 5.1: Build Admin Theme Configuration Page**
  ...
  - **Details:**
    - Form fields:
      ...
      - Secondary Accent Color (Color picker + Hex input)
      - Logo Upload / Dark Mode Logo Upload
...
1. **Unit Tests (.NET):**
   - `TenantBrandingTests.cs`: Validates hex regex parsing, defaults, and immutability.
   - `TenantAggregateTests.cs`: Validates `UpdateBranding` raises domain events.
2. **Integration Tests (.NET):**
   - `LookupTenantEndpointTests.cs`: Tests public lookup returns both primary and secondary colors with correct caching headers.
   - `UpdateTenantEndpointTests.cs`: Tests admin update persists to database.
3. **Frontend Tests (Vitest):**
   - `color-utils.test.ts`: Tests relative luminance and contrast ratio calculation across edge-case colors:
     - Pure White (`#FFFFFF`) -> Primary Foreground must be `#09090b`
     - Neon Yellow (`#FFFF00`) -> Primary Foreground must be `#09090b`
     - Deep Navy (`#0F172A`) -> Primary Foreground must be `#FFFFFF`
     - Crimson (`#A51C30`) -> Primary Foreground must be `#FFFFFF`
4. **End-to-End Verification:**
   - Configure Tenant A with Navy/Gold (`#1E3A8A` / `#F59E0B`).
   - Configure Tenant B with Emerald/Coral (`#059669` / `#F97316`).
   - Verify both academies load their custom identities simultaneously on separate subdomains with zero CSS pollution between them.
