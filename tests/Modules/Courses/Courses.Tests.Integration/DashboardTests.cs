...
public class DashboardTests : BaseIntegrationTest
{
    ...
    private async Task<Guid> CreatePublishedCourse(Guid tenantId, string title)
    {
        ...
        var videoId = await SeedAvailableVideoAsset(courseId, tenantId, "L1");
        await Client.PostAsJsonAsync($"/courses/{courseId}/pool/{videoId}/assign", new AssignAssetRequest
        {
            CourseId = courseId,
            AssetId = videoId,
            SectionId = sectionId,
